...
---
### Overview
We implemented the following features to ensure a robust and immersive gameplay experience:
-  **Ball Physics:** Simulated 3D parabolic trajectories using vector-based gravity and air resistance equations. 
-  **Collision Mechanics:** Implemented AABB logic for ball-to-racket interactions and established spatial coordinate thresholds to validate scoring zones.
-  **Skill System:** Integrated character-specific abilities governed by a centralized logic controller and cooldown timers.
-  **AI Logic:** Designed a multi-tiered difficulty system driven by a Finite State Machine and predictive movement.
-  **Map & Environmental Effects:** Implemented dynamic court visuals and particle effects to enhance visual feedback during high-impact plays.
-  **Responsive UI Design:** Utilized relative coordinate mapping to ensure full playability across all browser window sizes.
-  **Interactive Onboarding:** Built a sequential tutorial module to guide new players through core mechanics.
-  **Animation:** Synchronized state-driven sprite sheets with player actions for fluid visual feedback.
-  **Sound System:** Orchestrated a centralized audio manager for seamless BGM cross-fading and event-triggered SFX.
...
The primary challenge was simulating a realistic tennis ball trajectory within a 2D top-down environment. Standard 2D physics often feel "flat," lacking the vertical depth required for a sport like tennis. We had to implement a **Pseudo-3D Kinematic Model** that calculates height ($z$-axis) and translates it into visual screen space, while ensuring that the hitting mechanics felt responsive and dynamic.

#### **1. Kinematic Model for Trajectory Simulation**
Instead of simple linear movement, the ball’s position is governed by a discrete integration of Newtonian physics. We represent the ball's state as a 3D vector $\mathbf{P}(x, y, z)$. The vertical movement ($z$-axis) is simulated using the following equations for each frame $\Delta t$:

$$z_{t+\Delta t} = z_t + v_{z,t} \cdot \Delta t$$

$$v_{z,t+\Delta t} = v_{z,t} - g \cdot \Delta t$$

Where $g$ represents the simulated gravity constant. To render this on a 2D canvas, we mapped the 3D coordinates to 2D screen coordinates $\mathbf{P'}(x', y')$ using a height-offset projection:

$$\begin{cases} x' = x \\ y' = y - z \end{cases}$$

This creates the visual illusion of the ball "rising" and "falling." To enhance spatial awareness, we implemented a **Dynamic Shadow Scaling Function**, where the shadow's radius $R_s$ is inversely proportional to the height $z$:

$$R_s(z) = \max(R_{base} - \alpha \cdot z, R_{min})$$

#### **2. Responsive Hitting via Linear Mapping of Relative Offset**
A significant technical hurdle was ensuring that players could "aim" the ball based on their positioning. Rather than using static reflection, we implemented a **Transfer Function** that maps the relative horizontal offset between the ball and the paddle center to the exit velocity $v_x$.

When a collision is detected via **AABB (Axis-Aligned Bounding Box)**, the new horizontal velocity $v_{x, out}$ is calculated as:

$$v_{x, out} = (B_x - P_x) \cdot \lambda$$

Where:
* $B_x$ is the ball's x-coordinate.
* $P_x$ is the player's center x-coordinate.
* $\lambda$ is the `DIRECTION_MULT` coefficient.

This allows players to perform "angled shots" by hitting the ball with the edges of the racket. Furthermore, we integrated **Velocity Inheritance** during serves, where the ball’s initial $v_x$ is constrained to a specific sector based on the serving side, ensuring competitive fairness:
...
#### **3. Overcoming Discrete Collision Limitations (Tunneling)**
At high speeds, the ball risks "tunneling" (passing through the paddle between frames). We mitigated this by combining a high-frequency update loop with **Safety Boundary Checks** and **Velocity Normalization**. By monitoring the `bounceCount` and `isTossing` states, the system maintains a robust state machine that prevents illegal score triggers even during frame rate fluctuations, ensuring that every "PERFECT" hit is accurately registered and synchronized with the visual feedback system.

### **Technical Challenge 2: Strategic AI Behavior and Predictive Movement** (Pending)

The challenge was to develop an AI that provides a scalable difficulty curve without appearing "robotic" or "cheating" with impossible speeds. We achieved this by integrating **Probabilistic Prediction** with a **Strategic Behavior Model**.

#### **1. Predictive Interception and Error Modeling**
To simulate human anticipation, the AI calculates a target intercept $X_{target}$ based on the ball’s horizontal velocity $v_x$:

$$X_{target} = X_{ball} + (v_{x, ball} \cdot \Phi) + \epsilon$$

Where $\Phi$ is the **Prediction Window** and $\epsilon$ is a **Gaussian Noise** variable. This formula allows the AI to "read" the ball’s trajectory. The difficulty is primarily scaled by these two parameters: in **Easy Mode**, we increase $\epsilon$ (higher error) and $\Delta f$ (frame latency), causing the AI to frequently miscalculate the ball's path.

#### **2. Multi-tiered Finite State Machine (FSM)**
We managed AI behavior through an FSM to decouple decision-making from physics. The AI transitions between **Patrol** (idle with Perlin noise "fidgeting"), **Preparation** (calculating serve coordinates), and **Active Defense**. A specialized **Stunned State** was also implemented, where the AI's `stunTimer` is triggered by specific player skills, creating a window for the player to score.

#### **3. Strategic Pattern Selection**

This combination of **mathematical prediction** and **tactical variety** ensures that the AI feels like a competent opponent that adapts its strategy based on the player's position, rather than a simple reactive script.
